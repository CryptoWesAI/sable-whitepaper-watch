...
| [`status/counterfeit.jsonl`](status/counterfeit.jsonl) | The counterfeit watch. Every hour the watch asks DexScreener's public search (the queries SABL, SABLE, Sable) for tokens trading on Robinhood Chain (chain id 4663, DexScreener id `robinhood`) whose name or symbol is SABL or a wrapper of it (wSABL, stSABL, Sable, dotted or look-alike letters; a word like "reusable" does not count). A line only when the list of such tokens changes: time, addresses, each token's name, symbol, deepest pool, liquidity, when the pool opened, and how many look-alikes trade on other chains. The first line is the baseline (10 September 2026: one token, Sable/SABLE at `0xd00126f6f702367808d85bdEd7b41BDBD59e48cf`, a Uniswap pool opened 26 July 2026, before SABL existed). A new address gets a CHANGELOG entry. Sable Network's token exists only on Solana; nothing on Robinhood Chain is it. The chain's own explorer sits behind a bot wall, which is why the market's index is asked instead. |
